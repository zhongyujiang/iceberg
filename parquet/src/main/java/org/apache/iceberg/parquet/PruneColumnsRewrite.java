...
public class PruneColumnsRewrite {
  private static final Logger LOG = LoggerFactory.getLogger(PruneColumnsRewrite.class);

  private FileRewriteStatus fileRewriteStatus;
  private final Schema currentSchema;
  private final InputFile inputFile;
  private final boolean skipUnsupportedCols;
  private final NameMapping nameMapping;
  private MessageType rewriteSchema;
  private Configuration conf;

  public PruneColumnsRewrite(Table table, InputFile inputFile, boolean skipUnsupportedCols) {
    this.currentSchema = table.schema();
    this.inputFile = inputFile;
    this.skipUnsupportedCols = skipUnsupportedCols;
  ...
  public void selectRewriteSchema() {
    try (ParquetFileReader reader = ParquetFileReader.open(ParquetIO.file(inputFile))) {
      ...
      if (droppedIds.size() == fileIds.size()) {
        ...
        this.fileRewriteStatus = FileRewriteStatus.REMOVE;
        return;
      }

      if (droppedIds.isEmpty()) {
        // no columns are needed to be pruned
        this.fileRewriteStatus = FileRewriteStatus.RETAIN;
      }
      ...
      this.fileRewriteStatus = FileRewriteStatus.REWRITE;
      this.rewriteSchema = leftSchema;
    } catch (IOException e) {
  ...
  public void doExecute() {
    selectRewriteSchema();
    if (fileRewriteStatus != FileRewriteStatus.REWRITE) return;


  }


  public static void execute(Configuration conf, InputFile inputFile, OutputFile outputFile,
